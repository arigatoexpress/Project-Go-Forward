...
class FirestoreEmailLoginCodeStore:
    ...
    def get(self, email: str) -> EmailLoginCodeRecord | None:
        snap = self._collection.document(_email_doc_id(email)).get(timeout=FIRESTORE_RPC_TIMEOUT)
        if not snap.exists:
            return None
    ...
    def increment_attempts(self, email: str) -> int:
        ...
        doc_ref.update({"attempts": attempts}, timeout=FIRESTORE_RPC_TIMEOUT)
        return attempts

    def delete(self, email: str) -> None:
        self._collection.document(_email_doc_id(email)).delete(timeout=FIRESTORE_RPC_TIMEOUT)
