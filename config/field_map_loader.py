...
def get_field_map() -> Dict:
    ...
    if _field_map is None:
        with open(_FIELD_MAP_PATH, "r") as f:
            _field_map = json.load(f)
    return _field_map


def get_template_config(template_name: str) -> Optional[Dict]:
    """Get the full config for a specific template."""
    return get_field_map().get("templates", {}).get(template_name)


def get_template_field_map(template_name: str) -> Optional[Dict[str, str]]:
...
def get_packet_config(packet_name: str) -> Optional[Dict]:
