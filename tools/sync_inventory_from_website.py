...
from scraper import get_soup, scrape_inventory

logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)


def extract_detailed_home_info(detail_url: str) -> dict | None:
            ...
            try:
                ...
                if dry_run:
                    ...
                    results["homes"].append(
                        {"model_name": model_name, "action": action, "doc": firestore_doc}
                    )
                    if existing:
                        results["updated"] += 1
                    else:
...
def generate_sync_report(results: dict, dry_run: bool = True):
    ...
    report = {"mode": mode, "timestamp": timestamp, "results": results}
    ...
    with open(summary_path, "w") as f:
        ...
        for home in results["homes"]:
            ...
            f.write(
                f"    Specs: {doc.get('bedrooms')}BR/{doc.get('bathrooms')}BA, {doc.get('sqft')} sqft\n"
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Sync inventory from texashomeoutlet.com to Firestore"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    parser.add_argument(
        "--force", action="store_true", help="Actually write to database (use with caution)"
    )
