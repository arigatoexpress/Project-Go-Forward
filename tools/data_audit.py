...
def audit_inventory_data():
    ...
    for idx, row in df.iterrows():
        ...
        if (
            serial_str in ["Serial #", "nan", ""]
            or "Section" in serial_str
            or "Approval" in serial_str
        ):
            skipped_header += 1
    ...
    print(
        f"   TOTAL SKIPPED:            {len(skipped_rows):>4} ({len(skipped_rows)/total_rows*100:.1f}%)"
    )
    ...
    for idx, serial, model, msrp in valid_rows[:5]:
        print(
            f"   Row {idx+2}: {serial} | {model[:30]:<30} | ${msrp if not pd.isna(msrp) else 'N/A'}"
        )
    ...
    for _, serial, model, _ in valid_rows:
        ...
        if not re.search(r"\d+x\d+", model):
            missing_dims += 1
    print(
        f"   {'⚠️' if missing_dims > 0 else '✅'} Missing dimensions: {missing_dims}/{valid_records}"
    )
    ...
    print("\n✅ Audit complete!")
    print(
        f"   Recommendation: {'All good!' if valid_records > 50 and len(skipped_rows) < 20 else 'Review skipped records above.'}"
    )
