...
Environment:
    ...
    FIRESTORE_PROJECT_ID - GCP project ID (optional, uses default if not set)
"""
...
MANUFACTURER_MAP = {
    ...
    "Clayton Homes": {
        "name": "Clayton Homes", 
        "manufacturer_id": "clayton",
        "phone": None
    },
    "Fleetwood Homes": {
        "name": "Fleetwood Homes",
        "manufacturer_id": "fleetwood",
        "phone": None
    },
    "Champion Homes": {
        "name": "Champion Homes",
...
class InventorySync:
    ...
    def _extract_card_data(self, card) -> Optional[Dict]:
        ...
        try:
            ...
            # Determine if new based on manufacturer
            is_new = manufacturer in ["New Vision Manufacturing", "Jessup Housing", "Legacy Housing"]
    ...
    def _get_manufacturer_info(self, manufacturer_name: str, cdn_url: str = None, floorplan_url: str = None) -> Dict:
    ...
    def _get_plan_id(self, model_name: str) -> Optional[str]:
    ...
    def check_existing_inventory(self, inventory_id: str) -> Optional[Dict]:
    ...
    def sync_to_firestore(self, items: List[InventoryItem], force_update: bool = False):
        ...
        for item in items:
            try:
                ...
                doc_ref = self.db.collection('inventory').document(item.id)
    ...
    def run_sync(self, force_update: bool = False) -> Dict:
        ...
        preview_file = f"data/inventory_sync_preview_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def main():
    ...
    parser.add_argument('--scrape-only', action='store_true',
                        help='Only scrape website, do not sync to Firestore')
