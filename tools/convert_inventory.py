...
def convert_excel_to_json():
    ...
    )  # Up one level from tools/ to project root
    ...
    for _, row in df.iterrows():
        ...
        if (
            "Section" in serial_str
            or "Approval" in serial_str
            or "Days out" in serial_str
            or "wks" in serial_str
        ):
            continue
        ...
        try:
            invoice_amount = (
                float(invoice_amt)
                if invoice_amt and not pd.isna(invoice_amt) and str(invoice_amt) != "Invoice Amount"
                else None
            )
        except (ValueError, TypeError):
            invoice_amount = None
        ...
        inventory.append(
            {
                ...
                "manufacturer": str(row.get("Manufacturing Plant", ""))
                if not pd.isna(row.get("Manufacturing Plant"))
                else "Unknown",
                "model_name": model,
