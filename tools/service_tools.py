...
def check_warranty_status(
    purchase_date: str, issue_type: str = "Structural", tool_context: ToolContext = None
) -> dict:
    """
    ...
    if issue_type_lower == "cosmetic":
        ...
        return {
            ...
            "message": (
                ...
                if is_covered
                else f"The 30-day cosmetic warranty expired on {cosmetic_expires.strftime('%B %d, %Y')}. Cosmetic issues are no longer covered, but we can recommend trusted contractors for repairs."
            ),
        }
    ...
    return {
        ...
        "message": (
            ...
            if is_covered
            else f"The 1-year structural warranty expired on {structural_expires.strftime('%B %d, %Y')}. We can provide recommendations for licensed contractors in your area."
        ),
    }


def analyze_defect_image(
    image_url: str, defect_description: str, tool_context: ToolContext = None
) -> dict:
    """
    ...
    damage_keywords = [
        ...
        "stain",
        "pet",
        "water damage from leak I ignored",
    ]
    ...
    if any(
        ...
        for word in ["urgent", "emergency", "flooding", "no power", "no heat"]
    ):
        severity = "Critical"
    ...
    return {
        ...
        "results": {
            ...
            "reasoning": (
                ...
                + (
                    "This type of issue is typically covered under warranty."
                    if warranty_likely
                    else "This type of damage is typically not covered under warranty as it suggests customer responsibility."
                )
            ),
        },
        "next_steps": (
            "I'll proceed with creating a service request for warranty repair."
            if warranty_likely
            else "While this isn't covered under warranty, I can provide you with a list of trusted local contractors."
        ),
    }
...
def generate_invoice_pdf(
    ...
    work_order = {
        ...
        "customer": {"name": customer_name, "address": customer_address},
        "contractor": {"name": contractor_name, "email": contractor_email},
