...
class InventoryPictureAuditor:
        ...
        for home in website_homes:
            ...
            if result.asset_photos_count < 5:
                result.actions_needed.append(
                    f"Need more photos (only have {result.asset_photos_count})"
                )
    ...
    def _print_summary(self, report: dict):
        ...
        print(
            f"In database: {s['in_database']} ({s['in_database']/s['total_homes_on_website']*100:.1f}%)"
        )
        print(
            f"In asset catalog: {s['in_asset_catalog']} ({s['in_asset_catalog']/s['total_homes_on_website']*100:.1f}%)"
        )
