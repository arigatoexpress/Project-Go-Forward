...
This is purely deterministic URL-pattern detection — no ML, no manual
tagging. If a future CDN uses a different convention, extend
``FLOORPLAN_URL_PATH_TOKENS`` (single constant, one-line change).
"""
...
# Path tokens that mark a floorplan URL. Match is case-insensitive and
# checks for substring presence in the URL path, so query strings or
# differently-cased CDN hosts do not break detection.
FLOORPLAN_URL_PATH_TOKEN = "/floorplan/"
FLOORPLAN_URL_PATH_TOKENS: tuple[str, ...] = (FLOORPLAN_URL_PATH_TOKEN,)


def is_floorplan_url(url: str | None) -> bool:
    """Return True if ``url`` is a floorplan image URL.

    Detection is case-insensitive and based on path tokens. Empty / None
    inputs return False.
    """
    if not url or not isinstance(url, str):
        return False
    lowered = url.lower()
    return any(token in lowered for token in FLOORPLAN_URL_PATH_TOKENS)
