#!/usr/bin/env python3
"""Sanitized, read-only Google Ads account-access probe.
...
def _result(*, status: int | None, failure: str | None) -> dict[str, Any]:
    return {
...
def probe_access(
    *,
    ...
    try:
        ...
        response = requester(
            (
                ...
                f"customers/{customer}/googleAds:search"
            ),
            headers=headers,
            json={"query": QUERY},
            timeout=20,
    ...
    if response.status_code == 200:
        result = _result(status=200, failure=None)
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(json.dumps(result, indent=2, sort_keys=True))
    return 0 if result["account_access_validated"] else 1
