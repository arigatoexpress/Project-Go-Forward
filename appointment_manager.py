...
class AppointmentManager:
    ...
    def create_appointment_sync(self, appt: Appointment) -> Appointment:
        ...
        def txn_create(transaction):
            ...
            existing = list(query.stream(transaction=transaction, timeout=firestore_timeout()))
    ...
    async def get_appointment(self, appointment_id: str) -> Appointment | None:
        ...
        def _get():
            doc = self._collection().document(appointment_id).get(timeout=firestore_timeout())
            if doc.exists:
    ...
    async def get_appointments_by_date(self, date_str: str) -> list[Appointment]:
        ...
        def _query():
            ...
            return [
                ...
                for doc in q.stream(timeout=firestore_timeout())
            ]
