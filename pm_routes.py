...
async def list_projects(project_type: ProjectType | None = None, status: str | None = None):
...
async def list_tasks(
    ...
    return {"tasks": tasks[:limit], "total": len(tasks), "count": len(tasks[:limit])}
...
async def update_task(task_id: str, updates: dict, actor: str | None = None):
    """Update task fields"""
    if task_id not in tasks_db:
        raise HTTPException(status_code=404, detail="Task not found")
...
async def transition_task(task_id: str, new_state: WorkflowState, actor: str | None = None):
...
async def list_cycles(status: str | None = None):
    """List cycles"""
...
async def get_activity_feed(
    project_id: str | None = None, task_id: str | None = None, limit: int = Query(20, le=50)
):
    """Get recent activity across all projects"""
    activities = sorted(activities_db.values(), key=lambda a: a.created_at, reverse=True)
...
async def list_views(owner_id: str | None = None):
    """List saved views"""
...
async def get_dashboard():
    ...
    # Recent activity
    recent_activity = sorted(activities_db.values(), key=lambda a: a.created_at, reverse=True)[:10]
...
async def import_github_issues(project_id: str, repo: str, issues: list[dict]):
...
def _map_github_state(github_state: str) -> WorkflowState:
    ...
    mapping = {"open": WorkflowState.TODO, "closed": WorkflowState.DONE}
