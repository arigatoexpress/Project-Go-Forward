...
class ChatHistory:
    ...
    async def save_session(self, session: ChatSession):
        ...
        try:
            ...
            def _save():
                doc_ref.set(session.to_dict(), timeout=firestore_timeout())
    ...
    async def add_message(
        self, session_id: str, user_id: str, role: str, text: str, metadata: dict = None
    ) -> ChatMessage:
    ...
    async def get_user_sessions(
        self, user_id: str, limit: int = 10, status: str = None
    ) -> list[ChatSession]:
        ...
        def _stream():
            return [
                ChatSession.from_dict(d.to_dict())
                for d in query.stream(timeout=firestore_timeout())
            ]
    ...
    async def update_session_status(self, session_id: str, status: str, lead_id: str = None):
        ...
        def _update():
            doc_ref.update(updates, timeout=firestore_timeout())
    ...
    async def get_context_for_agent(
        ...
        formatted = []
        for msg in recent:
            formatted.append({"role": msg.role, "parts": [{"text": msg.text}]})
