...
async def test_edge_cases():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for idx, msg in enumerate(conversation_messages, 1):
            result = await send_message(
                session, "edge_user_long", session_id, msg, f"Long Conversation Msg {idx}"
            )
        ...
        result = await send_message(
            session, "edge_user_3", "edge_session_3", long_message, "Very Long Input"
        )
        print(f"  Result: {result['status']} ({result.get('duration', 'N/A')}s)")
        results.append(result)
    ...
    avg_duration = sum(r.get("duration", 0) for r in results if "duration" in r) / len(
        [r for r in results if "duration" in r]
    )
