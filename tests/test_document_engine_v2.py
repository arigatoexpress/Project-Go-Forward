...
def test_v2_maps_manufacturer_location_from_split_fields(monkeypatch, tmp_path):
    ...
    assert (
        captured["topmostSubform[0].Page1[0].Seller_Name[0]"]
        == " Prosperity Acquisitions, Inc. dba Texas Home Outlet"
    )
...
def test_v2_defaults_installer_identity_for_installation_warranty(monkeypatch, tmp_path):
    ...
    assert result["success"] is True
    assert (
        captured["topmostSubform[0].Page1[0].Installer_Name[0]"]
        == "Prosperity Acquisitions, Inc. dba Texas Home Outlet"
    )
    assert (
        captured["THO_TDHCA1124_Installer_Contact_Name"]
        == "Prosperity Acquisitions, Inc. dba Texas Home Outlet"
    )
